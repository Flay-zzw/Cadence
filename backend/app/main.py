...
def persist(content, model, pace=180):
    value = {'id': str(uuid4()), 'revision': 1, 'created_at': datetime.now(timezone.utc).isoformat(), 'pace': pace,
             'content': content.model_dump(), 'generation_meta': {'model': model, 'prompt_version': 'v1'}}
...
async def run_generation(identifier, request, config):
    ...
    try:
        ...
        project = persist(content, config['model'], request.pace)
