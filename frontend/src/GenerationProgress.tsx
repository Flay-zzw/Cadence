...
const stages=[['planning','素材分析与大纲'],['writing','逐页生成'],['validating','结构校验'],['saving','保存项目']];
function elapsed(start:string|null|undefined,end:string|null|undefined,now:number){
 ...
 if(seconds===0)return end?'少于 1 秒':'刚开始';
...
export default function GenerationProgress({job,connectionError}:{job:GenerationJob;connectionError:string}){
 ...
 const lastPage=job.pages?.find(p=>p.index===job.last_processed_page)||job.pages?.slice().reverse().find(p=>p.started_at);
 const current=job.pages?.find(p=>p.index===job.current_page)||(!active?lastPage:undefined);
 ...
 return <section className="generation-progress" aria-label="生成进度">
  ...
  <div className="progress-metrics">
   ...
   <div><span>{current?(active?'本页耗时':'最后一页耗时'):'本阶段耗时'}</span><strong>{current?elapsed(current.started_at,current.finished_at||job.finished_at,now):elapsed(currentStage?.started_at,currentStage?.finished_at||job.finished_at,now)}</strong></div>
  </div>
  ...
  {job.status==='completed'&&<p className="progress-note">结构校验与保存由本机完成，通常不足 1 秒；结构校验不代表事实核查。</p>}
